...
//                            Log.e("TAG", attrs.getAttributeName(i) + " , " + attrs.getAttributeValue(i));
    ...
    public static int getAttr(Class draw,String attrName) {
        if (attrName == null || attrName.trim().equals("") || draw == null) {
            return com.jaydenxiao.common.R.attr.colorPrimary;
        }
        try {
    ...
    public static void toggleThemeSetting(Activity ctx) {
        if(ChangeModeHelper.getChangeMode(ctx) == ChangeModeHelper.MODE_DAY){
            changeNight(ctx, com.jaydenxiao.common.R.style.NightTheme);
        }else if(ChangeModeHelper.getChangeMode(ctx) == ChangeModeHelper.MODE_NIGHT){
            changeDay(ctx, com.jaydenxiao.common.R.style.DayTheme);
        }
    }
    ...
    private static void refreshUI(Activity ctx) {
        ...
        theme.resolveAttribute(com.jaydenxiao.common.R.attr.colorPrimary, typedValue, true);
        View view = ctx.findViewById(com.jaydenxiao.common.R.id.action_bar);
        if(view!=null){
    ...
    private static void refreshStatusBar(Activity ctx) {
        if (Build.VERSION.SDK_INT >= 21) {
            TypedValue typedValue = new TypedValue();
            Resources.Theme theme = ctx.getTheme();
            theme.resolveAttribute(com.jaydenxiao.common.R.attr.colorPrimaryDark, typedValue, true);
